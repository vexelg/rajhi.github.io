            ...
            for (var signature in data) {
                if (data.hasOwnProperty(signature)) {
                    ...
                    if (data[signature].normalFiltering.length > 0) {
						...
						url = signature === 'poisearch' ? XA.component.search.url.createMultiFacetUrlWithoutFacetParam(requestData, data[signature].normalFiltering, signature) 
														: XA.component.search.url.createMultiFacetUrl(requestData, data[signature].normalFiltering, signature);
                        XA.component.search.ajax.getData({
                            callback: function (data) {
                                XA.component.search.vent.trigger("facet-data-filtered", data);
                            },
							/* Start: Ovrlode developer chnage the method from 'createMultiFacetUrl' to 'createMultiFacetUrlWithoutFacetParam' to exclude the facet paramter which will filter global search parametr list, if need to include the facet paramer in search URL, revert the method call */
                            url: url
							/* End: Ovrlode developer chnage the method from 'createMultiFacetUrl' to 'createMultiFacetUrlWithoutFacetParam' to exclude the facet paramter which will filter global search parametr list, if need to include the facet paramer in search URL, revert the method call */
                        });
                    }
                    ...
                    if (data[signature].partialFiltering.length > 0) {
                        _.each(data[signature].partialFiltering, function (facetName) {
							...
							url = signature === 'poisearch' ? XA.component.search.url.createMultiFacetUrlWithoutFacetParam(requestData, [facetName], signature)
															: XA.component.search.url.createMultiFacetUrl(requestData, [facetName], signature);
                            XA.component.search.ajax.getData({
                                callback: function (data) {
