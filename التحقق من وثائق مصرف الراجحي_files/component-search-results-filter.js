XA.component.search.facet.resultsfilter = (function ($, document) {
    ...
    var FacetResultsFilterView = XA.component.search.baseView.extend({
        ...
        updateFacet: function (param) {
            ...
            if (properties.multi) {
                ...
                if (facetValue) {
					...
					XA.connector.mapsConnector.FacetFilter=facetValue;
                    facetGroup.data('active-facet', hash);
                    this.setActiveFacet(facetName, facetValue);
					/*Start: Code added by ovrlod developer to handle the subtype category of branch and atm*/
					 var branchFacet = jQuery('.pws-branch-type'),
						atmsFacet = jQuery('.pws-atm-type');
					if(isNaN(facetValue))
					{
						if(facetValue.toLowerCase() === "atms" || facetValue.toLowerCase() === "kiosk" || facetValue === encodeURI("ماكينات الصراف الآلي") || facetValue === encodeURI("أجهزة أسرع"))
							{
							atmsFacet.addClass('force-show');
							branchFacet.removeClass('force-show');
							this.removeSubTypeFacet(branchFacet, atmsFacet);
							}
						if(facetValue.toLowerCase() === "branch" || facetValue === encodeURI("فرع"))
						{
							branchFacet.addClass('force-show');
							atmsFacet.removeClass('force-show');
							this.removeSubTypeFacet(atmsFacet,branchFacet);
						}
					}
					/*End: Code added by ovrlod developer to handle the subtype category of branch and atm*/
                    queryModel.updateHash(hash);
		...
		removeSubTypeFacet: function (facetToHide, facetToShow) {
           var    factSig = [];	   
		   
		  
		   if(facetToHide !== undefined)
		   {
			var facetData = facetToHide.data(),
				facetClose = facetToHide.find('.facet-heading > span'),
                facetValues = facetToHide.find('.facet-value');
				if(facetData !== undefined)
				{
					factSig[0]=  facetData.properties.searchResultsSignature.toLowerCase()+'_'+ facetData.properties.f.toLowerCase();	
					queryModel.updateHash(this.updateSignaturesHash(factSig, "", {}));								
				}			   
                    ...
                    if (!jQuery.isEmptyObject(_.pick(hash, sig))) {
                        ...
                        if (values) {
                            ...
                            for (var i = 0; i < escapedValues.length; i++) {
                                escapedValues[i] = encodeURI(escapedValues[i]);
                            }
