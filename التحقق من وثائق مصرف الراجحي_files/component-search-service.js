XA.component.search.service = (function ($, document) {
    ...
    SearchServiceModel = Backbone.Model.extend({
        ...
        getSearchResultsData: function(overrideProps) {
            ...
            _.each(searchResultsModels, function (searchModel) {
				...
				if(signature=="poisearch")
				{
				url=url.replace('sxa/search','branchsearch').replace('branch type','branchtype');
				 if(!location.hash)
					 {
				return;
					 }
					 if(XA.connector.mapsConnector.FacetFilter && isLang=="ar" && url.indexOf('branchesoratm=')>-1)
					 {
						 var currentUrl = url;
                         var url2 = new URL(currentUrl);
                         url2.searchParams.set("branchesoratm", XA.connector.mapsConnector.FacetFilter); // setting your param
                         url = url2.href;
					 }
				}
                if (!searchModel.checkBlockingRequest()) {
					searchModel.blockRequests(true);
					  if(XA.connector.mapsConnector.FirstLoad && signature=="poisearch")
					  {
					setTimeout(function() { 
       apiModel.getData({
	...
	XA.connector.mapsConnector.FirstLoad=false;
					}
					else{
						apiModel.getData({
                        callback: function(data) {
					...
					searchModel.blockRequests(false);
				}
            });
        },
